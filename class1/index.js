...
const getUserFromId = (req) => {
    const findUser = users.find((item) => Number(item.id) === Number(req));
    return findUser;
};
const getUser = () => {
...
const getUrlFromUser = () => {
    app.get("/users/:userId/urls", (req, res) => {
        ...
        if (user) {
            ...
            if (urlFromUser.length)
                res.send(urlFromUser);
...
const redirectUrl = () => {
    app.get("/urls/:urlId", (req, res) => {
        const findUrl = urls.find((item) => item.id === req.params.urlId);
        if (findUrl)
            res.redirect(findUrl.destination);
